...
const globalStyles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #F8F6F0; font-family: Georgia, serif; }
  .rx-input { font-family: 'Source Sans 3', sans-serif; font-size: 18px; width: 100%; padding: 18px 24px; border: 2px solid #D0CCC0; border-radius: 12px; background: white; outline: none; color: #1A1A18; transition: border-color 0.2s; }
  .rx-input:focus { border-color: #185FA5; }
  .rx-btn { font-family: 'Source Sans 3', sans-serif; font-size: 16px; font-weight: 600; background: #185FA5; color: white; border: none; border-radius: 10px; padding: 16px 36px; cursor: pointer; transition: background 0.2s; width: 100%; }
  .rx-btn:hover { background: #0C447C; }
  .rx-btn:disabled { background: #B4B2A9; cursor: not-allowed; }
  .suggest-item { font-family: 'Source Sans 3', sans-serif; padding: 12px 20px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; transition: background 0.15s; border-bottom: 1px solid #F0EDE5; }
  .suggest-item:hover { background: #F0EDE5; }
  .category-pill { font-family: 'Source Sans 3', sans-serif; font-size: 13px; font-weight: 500; padding: 6px 16px; border-radius: 20px; cursor: pointer; border: 1.5px solid #D0CCC0; background: white; transition: all 0.2s; color: #5F5E5A; }
  .category-pill.active { background: #185FA5; border-color: #185FA5; color: white; }
  .drug-card { background: white; border-radius: 12px; border: 1px solid #E5E2DA; padding: 16px 20px; cursor: pointer; transition: all 0.2s; display: flex; justify-content: space-between; align-items: center; }
  .drug-card:hover { border-color: #185FA5; box-shadow: 0 2px 12px rgba(24,95,165,0.12); }
  .go-btn { font-family: 'Source Sans 3', sans-serif; font-size: 13px; font-weight: 600; padding: 8px 18px; border-radius: 8px; border: 1.5px solid #185FA5; color: #185FA5; background: transparent; cursor: pointer; text-decoration: none; display: inline-block; transition: all 0.2s; }
  .go-btn:hover { background: #185FA5; color: white; }
  .tab-btn { font-family: 'Source Sans 3', sans-serif; font-size: 15px; font-weight: 500; padding: 12px 28px; border: none; background: transparent; cursor: pointer; color: #888780; border-bottom: 2px solid transparent; transition: all 0.2s; }
  .tab-btn.active { color: #185FA5; border-bottom-color: #185FA5; font-weight: 600; }
  .price-card { background: white; border-radius: 14px; border: 1px solid #E5E2DA; padding: 20px 24px; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; }
  .back-btn { font-family: 'Source Sans 3', sans-serif; font-size: 14px; color: #185FA5; background: none; border: none; cursor: pointer; padding: 0; }
`
  ...
  if (page === "home") {
    return (
      <>
        ...
        <header style={{ padding: "24px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #E5E2DA", background: "white" }}>
          ...
          <span style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780" }}>Prices last verified May 2025 · Not medical advice</span>
        </header>
        ...
        <section style={{ maxWidth: 720, margin: "0 auto", padding: "80px 24px 48px" }}>
          ...
          <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 18, color: "#5F5E5A", lineHeight: 1.7, marginBottom: 48, fontWeight: 300 }}>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
              <div style={{ flex: 1, position: "relative" }}>
                ...
                {suggestions.length > 0 && (
                  <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "white", border: "1.5px solid #D0CCC0", borderRadius: 12, overflow: "hidden", zIndex: 100, boxShadow: "0 8px 24px rgba(0,0,0,0.1)" }}>
                    {suggestions.map(name => (
                      <div key={name} className="suggest-item" onClick={() => handleSelect(name)}>
                        <div>
                          ...
                          <span style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780", marginLeft: 8 }}>{DRUGS[name].generic}</span>
                        </div>
            ...
            {selectedDrug && drug && (
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                <select style={{ flex: 1, fontFamily: "Source Sans 3, sans-serif", fontSize: 15, padding: "12px 16px", borderRadius: 10, border: "1.5px solid #D0CCC0", background: "white", color: "#1A1A18" }} value={selectedStrength} onChange={e => setSelectedStrength(e.target.value)}>
                  {drug.strengths.map(s => <option key={s}>{s}</option>)}
                </select>
                <select style={{ flex: 1, fontFamily: "Source Sans 3, sans-serif", fontSize: 15, padding: "12px 16px", borderRadius: 10, border: "1.5px solid #D0CCC0", background: "white", color: "#1A1A18" }} value={selectedQty} onChange={e => setSelectedQty(e.target.value)}>
        ...
        <section style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px 80px" }}>
          <div style={{ borderTop: "1px solid #E5E2DA", paddingTop: 40, marginBottom: 24 }}>
            ...
            {filteredDrugs.map(name => (
              <div key={name} className="drug-card" onClick={() => { handleSelect(name); setPage("results") }}>
                <div>
                  ...
                  <div style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780", marginTop: 2 }}>{DRUGS[name].generic}</div>
                </div>
        ...
        <footer style={{ borderTop: "1px solid #E5E2DA", padding: "24px 40px", background: "white", textAlign: "center" }}>
          <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 12, color: "#888780", lineHeight: 1.8 }}>
  ...
  if (page === "results" && drug) {
    ...
    return (
      <>
        ...
        <header style={{ padding: "20px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #E5E2DA", background: "white" }}>
          ...
          <span style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780" }}>Prices are estimates · Verify at source</span>
        </header>
        ...
        <main style={{ maxWidth: 860, margin: "0 auto", padding: "40px 24px" }}>
          <div style={{ marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
              <div>
                ...
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 16, color: "#5F5E5A" }}>Generic: {drug.generic} · {selectedStrength} · {selectedQty}</p>
              </div>
              <div style={{ display: "flex", gap: 12 }}>
                <select style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 14, padding: "8px 14px", borderRadius: 8, border: "1.5px solid #D0CCC0", background: "white", color: "#1A1A18" }} value={selectedStrength} onChange={e => setSelectedStrength(e.target.value)}>
                  {drug.strengths.map(s => <option key={s}>{s}</option>)}
                </select>
                <select style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 14, padding: "8px 14px", borderRadius: 8, border: "1.5px solid #D0CCC0", background: "white", color: "#1A1A18" }} value={selectedQty} onChange={e => setSelectedQty(e.target.value)}>
                  {drug.quantities.map(q => <option key={q}>{q}</option>)}
                </select>
              </div>
            </div>
          ...
          <div style={{ borderBottom: "1px solid #E5E2DA", marginBottom: 28, display: "flex" }}>
          ...
          {resultTab === "cash" && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780", marginBottom: 16 }}>All legal, licensed US pharmacies and discount programs</p>
                {cashSources.map(src => {
                    ...
                    <div key={src.key} className="price-card">
                      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        ...
                        <div>
                          ...
                          {src.key === "goodrx" && zip && <div style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 12, color: "#888780" }}>Localized for ZIP {zip}</div>}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#888780", marginBottom: 4 }}>Often 50–80% lower. Personal-use imports are generally allowed under FDA enforcement discretion.</p>
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 12, color: "#B4B2A9", marginBottom: 16 }}>PharmacyChecker verifies international pharmacies meet safety standards.</p>
                {intlSources.map(src => {
                  const info = drug.sources[src.key]
                  if (!info) return null
                  return (
          ...
          {resultTab === "insurance" && (
            <div>
              {[
                ...
                <div key={item.title} style={{ background: "white", border: "1px solid #E5E2DA", borderRadius: 14, padding: "24px 28px", marginBottom: 16 }}>
                  <h3 style={{ fontFamily: "Playfair Display, serif", fontSize: 18, fontWeight: 600, color: "#1A1A18", marginBottom: 8 }}>{item.title}</h3>
                  <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 14, color: "#5F5E5A", marginBottom: item.extra ? 8 : 16, lineHeight: 1.6 }}>{item.note}</p>
                  {item.extra && <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 14, color: "#5F5E5A", marginBottom: 16, lineHeight: 1.6 }}>{item.extra}</p>}
                  <a href={item.url} target="_blank" rel="noopener noreferrer" className="go-btn">{item.cta}</a>
                </div>
              ))}
              <div style={{ background: "white", border: "1px solid #E5E2DA", borderRadius: 14, padding: "24px 28px", marginBottom: 16 }}>
                ...
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 14, color: "#5F5E5A", marginBottom: 4, lineHeight: 1.6 }}>{drug.sources.manufacturer.name}</p>
              ...
              <div style={{ background: "#F0EDE5", border: "1px solid #D0CCC0", borderRadius: 14, padding: "20px 24px" }}>
                <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 13, color: "#5F5E5A", lineHeight: 1.7 }}>
        ...
        <footer style={{ borderTop: "1px solid #E5E2DA", padding: "20px 40px", background: "white", textAlign: "center", marginTop: 40 }}>
          <p style={{ fontFamily: "Source Sans 3, sans-serif", fontSize: 12, color: "#888780" }}>
